...
def get_bot_file():
    ...
    # Always use bot.py based on file structure
    return "bot.py"
...
async def create_room_and_token() -> tuple[str, str]:
    """Helper function to get a Daily room URL from env and generate an access token.

    Returns:
        tuple[str, str]: A tuple containing (room_url, token)

    Raises:
        HTTPException: If DAILY_SAMPLE_ROOM_URL is not set or token generation fails
    """
    room_url = os.getenv("DAILY_SAMPLE_ROOM_URL")
    if not room_url:
        raise HTTPException(
            status_code=500,
            detail="DAILY_SAMPLE_ROOM_URL environment variable not set. Cannot create room dynamically."
        )

    # We have a room_url, now get a token for it
...
async def start_agent(request: Request):
    ...
    try:
        ...
        # Use python directly and include LOCAL_RUN=1
        command = f"LOCAL_RUN=1 python {bot_file} -u {room_url} -t {token}"
        proc = subprocess.Popen(
            [command], # Pass command as a list for shell=True safety, though not strictly needed here
            shell=True,
            bufsize=1,
